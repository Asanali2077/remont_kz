        ...
        <div className="bg-gradient-to-r from-primary to-primary/80 print:from-blue-600 print:to-blue-500 px-8 py-8 text-white">
          <div className="flex items-start justify-between">
            ...
            <div className="text-right">
              ...
              <div className="text-white/70 text-xs mt-1">{t("createdDate", { date: formatDateShort(req.createdAt) })}</div>
            </div>
          </div>
        ...
        <div className="px-8 py-7 space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="bg-gray-50 dark:bg-gray-800/50 print:bg-gray-50 rounded-xl p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-bold text-sm uppercase tracking-wide text-muted-foreground">{t("client")}</span>
              </div>
            ...
            <div className="bg-gray-50 dark:bg-gray-800/50 print:bg-gray-50 rounded-xl p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-bold text-sm uppercase tracking-wide text-muted-foreground">{t("contractor")}</span>
              </div>
              {req.company ? (
                <>
                ...
                <p className="text-muted-foreground text-sm">{t("noContractor")}</p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="divide-y divide-border/50 border border-border/50 rounded-xl overflow-hidden">
              ...
              {req.city && <RowIcon label={t("city")} value={req.city} icon={<MapPin className="h-3.5 w-3.5 text-muted-foreground" />} />}
              {req.deadline && <RowIcon label={t("deadline")} value={formatDateShort(req.deadline)} icon={<Calendar className="h-3.5 w-3.5 text-muted-foreground" />} />}
                ...
                return (
                  <div key={i} className="relative mb-4 last:mb-0">
                    <div className={`absolute -left-6 h-5 w-5 rounded-full border-2 flex items-center justify-center ${
                      item.done ? "border-primary bg-primary text-white" : "border-border bg-background text-muted-foreground/30"
                    }`}>
              ...
              <div className="bg-amber-50 dark:bg-amber-950/20 print:bg-amber-50 border border-amber-200/50 dark:border-amber-800/30 rounded-xl p-5 space-y-3">
                ...
                {req.companyReply && (
                  <div className="pl-4 border-l-2 border-primary/30 mt-2">
                    <p className="text-xs text-muted-foreground font-semibold mb-1">{t("companyReply")}:</p>
          ...
          <div className="border-t border-border/50 pt-6 flex items-center justify-between text-xs text-muted-foreground">
            <span>{t("marketplaceDesc")}</span>
            <span>{t("docGenerated", { date: formatDateShort(new Date().toISOString()) })}</span>
          </div>
        </div>
      </div>
